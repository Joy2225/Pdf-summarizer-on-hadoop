...
db = client['BDA']
collection = db['Project_100']  # Assuming your collection is named 'links'
...
for idx, doc in enumerate(documents, start=1):
    ...
    pdf_url = doc.get('url')
    name = doc.get('name')
    file_path = os.path.join('input2', name)
...
def extractText(file):
    ...
    try:
        ...
        for page in pdfPages:
            text += page
        with open("extracted2/"+file[7:-4]+".pdf", "wb") as file:
...
total_time=0
for i in os.listdir("input2"):
    if i.endswith(".pdf"):
        text = extractText("input2/" + i)
        if text == -1:
            continue

# start_total = time.time()
for i in os.listdir("extracted2"):
    if i.endswith(".pdf"):
        with open("extracted2/"+i, "rb") as file:
            text = file.read()
        ...
        # print(total_time)
        with open("summary1/"+i[:-4]+"_summary.txt", "w") as file:
            file.write(summary)
